...
public class WeekDView extends View {
    ...
    private int mCurrentTimeTextColor = Color.rgb(99, 205, 43);//时间线
    private int mHourSeparatorHeight = 2;//小时上下空白高度
...
//        mStartDate = (Calendar) mToday.clone();
        ...
        mHeaderHeight = mHeaderTextHeight + mHeaderRowPadding;
    }
...
//        canvas.drawRect(0, 155, mTimeTextWidth + mHeaderColumnPadding * 2, 0, mHeaderBackgroundPaint);
...
//                mCurrentOrigin.y = -(mHourHeight * 24 + 0 - getHeight() + 0);
//
//            else mCurrentOrigin.y -= mDistanceY;
//        }
//        // 不要让视图高于/低于滚动的限制。设置滚动条的最大值和最小值。 调整45部分更高左侧
        if (mCurrentScrollDirection == Direction.VERTICAL) {
            if (mCurrentOrigin.y - mDistanceY > mHeaderHeight + mHeaderTextHeight + 20) {
                mCurrentOrigin.y = mHeaderHeight + mHeaderTextHeight + 20;
                Log.e("----------------->", "这是向下拉动,到最顶部");
            } else if (mCurrentOrigin.y - mDistanceY < -(mHourHeight * 24 + 20 - getHeight())) {
                mCurrentOrigin.y = -(mHourHeight * 24 + 20 - getHeight());
                Log.e("----------------->", "滑动到最下方继续滑动");
            } else if (mCurrentOrigin.y > getHeight()) {
                mCurrentOrigin.y = (mHourHeight * 24 + mHourHeight);
                Log.e("----------------->", "这是第三行");

//            }
//            else if  (mCurrentOrigin.y+mDistanceY<mHeaderHeight+ mHeaderTextHeight+80) {
//                mCurrentOrigin.y =mHeaderHeight+ mHeaderTextHeight+80;
////                Log.e("----------------->", "这是最后一行:"+mCurrentOrigin.y);
            } else {
        ...
        // 绘制标题栏的背景颜色。
        canvas.drawRect(0, mHeaderMarginBottom + mHeaderHeight, mHeaderColumnWidth,

                getHeight() + mHeaderHeight + 120, mHeaderColumnBackgroundPaint);
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (top < getHeight() && top > mHeaderMarginBottom + mHeaderHeight + 30)
...
//            float top = (mHourHeight * 24 / getHeight()) * (60 * now.get(Calendar.HOUR_OF_DAY) + now.get(Calendar.MINUTE))
//                    + mTimeTextHeight + mCurrentOrigin.y;
//            float tt=now.get(Calendar.MINUTE);
            float tt=now.get(Calendar.HOUR_OF_DAY)+(float)now.get(Calendar.MINUTE)/(float)60;
            float top = mCurrentOrigin.y + mHourHeight * (tt)+ mHeaderMarginBottom;

            if (top < getHeight() && top > mHeaderMarginBottom + mHeaderHeight + 200) {
                canvas.drawText(time, mTimeTextWidth + mHeaderColumnPadding, top+mTimeTextHeight, mCurrentTimeTextPaint);
            }
        }
    }
...
//                        mCurrentOrigin.x = mCurrentOrigin.x + (mWidthPerDay + mColumnGap) * 7;
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mWidthPerDay + startPixel - start > 0)
                canvas.drawRect(start, mTimeTextHeight / 2 + mHeaderMarginBottom + 80, startPixel + mWidthPerDay, getHeight(), sameDay ? mTodayBackgroundPaint : mDayBackgroundPaint);
            // 为时间绘制分隔线
            int i = 0;
            for (int hourNumber = 0; hourNumber < 24; hourNumber++) {
                float top = mCurrentOrigin.y + mHourHeight * hourNumber + mTimeTextHeight / 2 + 20;
        ...
        canvas.drawRect(0, 0, getWidth(), mHeaderHeight + mHeaderTextHeight + 40, mHeaderBackgroundPaint);
        Paint mLine = new Paint();
        mLine.setColor(mHeaderRowBackgroundColor);
        canvas.drawLine(0, mHeaderHeight + mHeaderTextHeight + 25, getWidth(), mHeaderHeight + mHeaderTextHeight + 25, mHourSeparatorPaint);
        ...
        for (int dayNumber = leftDaysWithGaps + 1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            canvas.drawText(StringData(day), startPixel + mWidthPerDay / 2, mHeaderTextHeight + 5, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);

            canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding + 40, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);

            startPixel += mWidthPerDay + mColumnGap;
        }
        canvas.drawRect(0, 0, mWidthPerDay, mHeaderHeight + mHeaderTextHeight + 24, mHeaderBackgroundPaint);
    }
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    if (bottom > 0 + mHeaderMarginBottom + mTimeTextHeight / 2 && left < right &&
                        ...
                        mEventBackgroundPaint.setAlpha(125);
