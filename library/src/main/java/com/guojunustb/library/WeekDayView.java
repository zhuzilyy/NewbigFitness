...
public class WeekDayView extends View {
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            Log.e("原点1:", mScroller.getCurrX() + "-----------" + e1.getX() + "");
            mDistanceX = distanceX;
            mDistanceY = distanceY;
            if (mCurrentScrollDirection != Direction.VERTICAL) {
                if (abs(x) > 200 && mAreDimensionsInvalidF && abs(moveLength) > 80) {
//                    oldX = mScroller.getCurrX() - e1.getX();
                    mAreDimensionsInvalidX = true;
//                    mScroller.startScroll((int) -x,0,getWidth()-mColumnGap,0);
                    mAreDimensionsInvalidF = false;
                    invalidate();
//                Log.e("原点:", oldX + "");
                    return false;
                }
            } else if (mCurrentScrollDirection ==Direction.VERTICAL && mCurrentScrollDirection != Direction.HORIZONTAL) {
...
//                mScroller.fling((int) mCurrentOrigin.x, 0, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) (getWidth()-mWidthPerDay), (int) (getWidth()-mWidthPerDay));
            }
...
//        canvas.drawRect(0, 0, mTimeTextWidth + mHeaderColumnPadding * 2, 0, mHeaderBackgroundPaint);
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
        ...
        if (sameDay) {
            ...
            if (top < getHeight() && top > mHeaderMarginBottom + 200) {
...
//                 mAreDimensionsInvalid=false;
            if (mAreDimensionsInvalidX) {
                ...
                Log.d("+++++++++++++++:",mWidthPerDay+"");
                if (mDistanceX > 0) {
                    mCurrentOrigin.x -=mWidthPerDay;
//                    mCurrentOrigin.x -=(getWidth()-mWidthPerDay-mColumnGap);
                } else {
                    mCurrentOrigin.x +=mWidthPerDay;
//                    mCurrentOrigin.x +=(getWidth()-mWidthPerDay-mColumnGap);
                }
